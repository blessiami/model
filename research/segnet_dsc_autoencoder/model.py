...
class Model(object):
    def __init__(self, criterion, device, opt, train_loader=None, test_loader=None, log_dir='./logs', resume=None):
        ...
        self._train_metric = Metric(train_loader.dataset.n_category, device) if train_loader else None

        self._test_loader = test_loader
        self._test_metric = Metric(test_loader.dataset.n_category, device) if test_loader else None
    ...
    def train(self, net, optimizer, epoch, scheduler):
        ...
        for idx, mini_batch in tqdm(enumerate(self._train_loader)):
            imgs, labels = mini_batch['image'].to(self._device), mini_batch['image'].to(self._device)
            ...
            with torch.no_grad():
                ...
                predictions = outputs # the maximum category index of outputs
                # _, predictions = net.module.prediction(outputs)  # the maximum category index of outputs
    ...
    def log(self, tags, loss, epoch, learning_rate=None):
        if self._opt.local_rank != 0:
            return
