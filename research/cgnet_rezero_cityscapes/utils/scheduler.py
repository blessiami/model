...
class CosineLR(_Scheduler):
    def __init__(self, optimizer, max_step, warm_start=0, last_epoch=-1):
        self._max_step = max_step
    ...
    def get_lr(self):
        ...
        return [last_epoch / self._warm_start * base_lr if self._warm_start > self.last_epoch
                else 0.5 * (1 + cos((last_epoch - self._warm_start) * pi / self._max_step)) * base_lr
                for base_lr in self.base_lrs]
